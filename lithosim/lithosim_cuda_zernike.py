...
def radial_poly(n, m, r):
    """
    Вычисляет радиальную часть Zernike: R_n^m(r)
    n: порядок
    m: частота (|m| <= n, (n - m) чётное)
    r: радиус, тензор
    """
    m = abs(m)
...
def zernike(n, m, r, theta):
    """
    Полиномы Zernike Z_n^m(r, theta)
    """
    R = radial_poly(n, m, r)
    if m > 0:
...
def generate_zernike_phase(shape, coeffs, device='cpu'):
    """
    Генерирует фазовую маску φ(u,v) из линейной комбинации настоящих Zernike
    shape: (H, W)
    coeffs: список коэффициентов [a0, a1, ..., aN]
    """
    H, W = shape
